    ...
    if (item) {
      document.getElementById('itemDetails').innerHTML = `
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <!-- Item Image -->
          <img src="${item.image}" class="w-full h-auto max-h-[500px] rounded-lg object-cover" alt="${item.name}">

          <!-- Item Details -->
          <div>
            <h1 class="text-4xl font-bold text-red-600 mb-4">${item.name}</h1>
            <p class="text-gray-600 mb-6">${item.description}</p>
            <p class="text-2xl font-semibold text-black mb-4">Price: <span class="font-bold">${item.price} EGP</span></p>
